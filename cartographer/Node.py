...
class Graph:
    ...
    def add_edge_by_indices(self, index1: int, index2: int, weight: float):
        self._adjacency_list[index1].append((index2, weight))
        self._adjacency_list[index2].append((index1, weight))

    def add_edge_by_name(self, name1: str, name2: str, weight: float):
        i1 = self._name_to_index[name1]
        i2 = self._name_to_index[name2]
        self.add_edge_by_indices(i1, i2, weight)
    ...
    def is_usable_index(self, index: int, accessible: bool, use_closed_corridors: bool) -> bool:
        node = self._nodes[index]
        return (
                (not accessible or node.is_accessible()) and
